...
const mergeSortWithCallStack = (arr, callStack) => {
  callStack.push(`mergeSort([${arr.toString()}])`);
  ...
  if (arr.length < 2) {
    ...
    let leftPart = mergeSortWithCallStack(arr.slice(0, mid), callStack);
    let rightPart = mergeSortWithCallStack(arr.slice(mid), callStack);
...
// Testing....
console.log(
  mergeSortWithCallStack(
    [4, 2, 765, 12, -434, 545.7, 423.54, -454.6, 0, 34],
    []
  )
);
